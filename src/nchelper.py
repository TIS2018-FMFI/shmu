...
class NcPollutantHelper(NcHelper):
    def __init__(self,pollutant_path):
        '''
        :param pollutant_path: netcdf pollutant path
        '''
        super(NcPollutantHelper, self).__init__(pollutant_path)
    ...
    def getRasterAtDateTime(self, datetime):
        ...
        return np.array(self.ds.variables['NO2'][idx,0,:,:])


class NcGridHelper(NcHelper):
    def __init__(self,grid_path):
        '''
        :param grid_path: netcdf grid path
        '''
        super(NcGridHelper, self).__init__(grid_path)

    def getStartCoords(self):
        '''
        :return: starting point
        '''
        return [np.array(self.ds.variables['LAT'][0,0,:,:]).mean(), np.array(self.ds.variables['LON'][0,0,:,:]).mean()]
